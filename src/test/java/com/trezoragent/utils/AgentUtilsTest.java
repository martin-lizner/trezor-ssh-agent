...
public class AgentUtilsTest {
    ...
    public void testTrezorSignatureValidation() throws Exception {
        byte[] uncompressedNistpKeyFromSSHKeyTrezor = AgentUtils.unframeUncompressedNistpKeyFromSSHKey(pubKeySSHTrezor);
        boolean validSignatureTrezor = IdentityUtils.isValidSignature(uncompressedNistpKeyFromSSHKeyTrezor,
                challengeTrezor, AgentUtils.createDERSignResponse(signatureTrezor));

        assertThat(validSignatureTrezor).isEqualTo(true);

    }

    @Test
    public void testTrezorSSHKeyConversion() throws Exception {
        ECPublicKey publicKeyFromBytes = IdentityUtils.getPublicKeyFromBytes(pubKeyTrezor);
        String decompressSSHKeyFromNistp256 = IdentityUtils.decompressSSHKeyFromNistp256(publicKeyFromBytes);
        byte[] sshKey = Base64.decode(decompressSSHKeyFromNistp256);

        assertThat(Arrays.equals(sshKey, pubKeySSHTrezor)).isEqualTo(true);
    }

    @Test
    public void testKeepKeySignatureValidation() throws Exception {
        byte[] uncompressedNistpKeyFromSSHKey = AgentUtils.unframeUncompressedNistpKeyFromSSHKey(pubKeySSHKeepKey);
        boolean validSignatureKeepKey = IdentityUtils.isValidSignature(uncompressedNistpKeyFromSSHKey,
                challengeKeepKey, AgentUtils.createDERSignResponse(signatureKeepKey));

        assertThat(validSignatureKeepKey).isEqualTo(true);
    }

    @Test
    public void testKeepKeySSHKeyConversion() throws Exception {
        ECPublicKey publicKeyFromBytes = IdentityUtils.getPublicKeyFromBytes(pubKeyKeepKey);
        String decompressSSHKeyFromNistp256 = IdentityUtils.decompressSSHKeyFromNistp256(publicKeyFromBytes);
        byte[] sshKey = Base64.decode(decompressSSHKeyFromNistp256);

        assertThat(Arrays.equals(sshKey, pubKeySSHKeepKey)).isEqualTo(true);
    }
