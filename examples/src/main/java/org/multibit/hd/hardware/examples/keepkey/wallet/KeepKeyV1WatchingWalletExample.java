...
import org.multibit.hd.hardware.examples.common.wallet.WatchingPeerEventListener;
 ...
 *  
 */
public class KeepKeyV1WatchingWalletExample {

  private static final Logger log = LoggerFactory.getLogger(KeepKeyV1WatchingWalletExample.class);
  ...
  private Wallet keepkeyWatchingWallet;
  ...
  public static void main(String[] args) throws Exception {
    ...
    KeepKeyV1WatchingWalletExample example = new KeepKeyV1WatchingWalletExample();
  ...
  private void createWatchingWallet(DeterministicHierarchy hierarchy) {
    ...
    try {
      ...
      if (walletFile.exists()) {
        ...
        keepkeyWatchingWallet = Wallet.loadFromFile(walletFile);
        ...
        keepkeyWatchingWallet = new Wallet(networkParameters, keyChainGroup);

        // Immediately save
        keepkeyWatchingWallet.saveToFile(walletFile);

      }

      log.debug("Example wallet = \n{}", keepkeyWatchingWallet.toString());
      ...
      blockChain.addWallet(keepkeyWatchingWallet);

      log.debug("Creating peer group...");
      createNewPeerGroup(keepkeyWatchingWallet);
    ...
    log.info("Wallet after sync: {}", keepkeyWatchingWallet.toString());
    ...
    try {
      keepkeyWatchingWallet.completeTx(sendRequest);
  ...
  private File createWalletEnvironment() throws IOException {
    ...
    File source = new File("./examples/src/main/resources/multibit-hardware.checkpoints");
  ...
  private Map<Integer, ImmutableList<ChildNumber>> buildReceivingAddressPathMap(Transaction tx) {
    ...
    for (int i = 0; i < tx.getInputs().size(); i++) {
      ...
      DeterministicKey keyFromPubKey = keepkeyWatchingWallet.getActiveKeychain().findKeyFromPubKey(data);
  ...
  private Map<Address, ImmutableList<ChildNumber>> buildChangeAddressPathMap(Transaction tx) {
    ...
    DeterministicKeyChain activeKeyChain = keepkeyWatchingWallet.getActiveKeychain();
    ...
    for (int i = 0; i < tx.getOutputs().size(); i++) {
      ...
      if (script.isSentToRawPubKey()) {
        ...
        if (keepkeyWatchingWallet.isPubKeyMine(pubkey)) {
      ...
      } else if (script.isPayToScriptHash() && keepkeyWatchingWallet.isPayToScriptHashMine(script.getPubKeyHash())) {
        ...
        byte[] pubkeyHash = script.getPubKeyHash();
        if (keepkeyWatchingWallet.isPubKeyHashMine(pubkeyHash)) {
          key = Optional.fromNullable(activeKeyChain.findKeyFromPubHash(pubkeyHash));
          address = Optional.fromNullable(new Address(MainNetParams.get(), pubkeyHash));
        }
      }
